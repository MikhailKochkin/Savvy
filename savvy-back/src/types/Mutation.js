...
const Mutation = mutationType({
  ...
  definition(t) {
    ...
    t.field("sendEmailToStudent", {
      ...
      resolve: async (_, { id, reminders, comment }, ctx) => {
        const updateCourseVisit = await ctx.prisma.courseVisit.update({
          where: { id },
          data: { reminders: { set: reminders } },
        });
        ...
        // console.log(courseVisits[0].coursePage.lessons);

        //         if (comment === "hello") {
        //           const NextWeek = await client.sendEmail({
        //             From: "Anastasia@besavvy.app",
        //             To: users[0].email,
        //             Subject: "Привет от твоего личного помощника из BeSavvy 😁",
        //             HtmlBody: NextWeekEmail.NextWeekEmail(
        //               users[0].name,
        //               `<p>Это Настя, менеджер курсов в BeSavvy.</p>
        // <p>Рада приветствовать тебя среди участников курса "${courseVisits[0].coursePage.title}".</p>
        // <p>Главное, запомни, что ты занимаешься не в одиночку. Я всегда буду рада помочь тебе с любым учебным или техническим вопросом. Со мной можно связаться, написав в Discord или ответив на это письмо.</p>
        // <p><b><i>Вы не будете расти, если не будете пытаться совершить что-то за пределами того, что вы уже знаете в совершенстве.</i></b> Ральф Эмерсонs</p>
        // `,
        //               courseVisits[0].coursePage.title,
        //               courseVisits[0].coursePage.id
        //             ),
        ...
        //             From: "Anastasia@besavvy.app",
        //             To: users[0].email,
        //             Subject: "Если нужна помощь, твой BeSavvy куратор всегда рядом 🖐🏻",
        //             HtmlBody: NextWeekEmail.NextWeekEmail(
        //               users[0].name,
        //               `<p>Это Настя, менеджер курсов BeSavvy.</p>
        // <p>Я увидела тебя среди участников курса "${courseVisits[0].coursePage.title}". Заметила, что что-то пошло не так и у тебя пока не получилось пройти курс до конца. Может быть, я могу чем-то помочь? </p>
        ...
        // <p><b><i>Ты не можешь ни выиграть, ни проиграть до тех пор, пока ты не участвуешь в гонках.</i></b> Дэвид Боуи</p>`,
        //               courseVisits[0].coursePage.title,
        //               courseVisits[0].coursePage.id
        //             ),
        //           });
        ...
        //               users[0].name,
        //               `<p>Это Настя, менеджер курсов BeSavvy.</p>
        // <p>Ты быстро движешься по курсу "${courseVisits[0].coursePage.title}". Еще пара уроков и ты дойдешь до конца. Молодец!</p>
        // <p><b><i>Никогда не путайте движения с действиями.</i></b> Э. Хемингуэй</p>`,
        //               courseVisits[0].coursePage.title,
        //               courseVisits[0].coursePage.id
        //             ),
        //           });
