...
class ChooseCareer extends Component {
    ...
    render() {
        return (
            <Styles>
                <Header>Чтобы получить максимальную пользу от Savvvy, 
                    выберите свой карьерный трек: </Header>
                    <User>
                {({data: {me}}) => (
                    ...
                    <Label>
                        <div>
                            ...
                            <Button
                                onClick={ async e => {
                                    e.preventDefault()
                                    const res1 = await this.setState({ careerTrackID: "cjwx79iaj00rk0712tz12j7vi"})      
                    ...
                    <Label>
                        <div>
                            ...
                            <Button
                              onClick={ async e => {
                                e.preventDefault()
                                const res1 = await this.setState({ careerTrackID: "cjwx78u7700rb07121pelqctm"})      
                                const res2 = await this.updateUser(e, updateUser)
                                Router.push({ pathname: '/courses'})
                          } }
                            >Выбрать</Button>
